...
def api_predict(test_json):
    ...
    for variable in categorical_variables:
        test[variable] = categorical_to_numeric(test, variable, "loan_status")
    
    # normalization
    test = normalize_variables(test, "standard")
    
    # getting the indexes separated out
    ids = test.index
    ...
    scores = pd.DataFrame(list(zip(ids, logit_predictions, rf_predictions, xg_predictions)))
    scores.columns = ["id", "logit", "rf", "xg"]
